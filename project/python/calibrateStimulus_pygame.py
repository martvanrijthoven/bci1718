...
def drawTrial(nSymbs,tgtSeq=None):
    ...
    for ti in range(0,nSymbs):
        ...
        if t_state<=0 : pygame.draw.polygon(windowSurface, t_color, pointlist_left)
        if t_state>0 : pygame.draw.polygon(windowSurface, t_color, pointlist_right)
...
def drawBlank(prime):
    ...
    pygame.draw.polygon(windowSurface, bgColor, ((0,height/2), (width/6, height/2+height/4), (width/6, height/2-height/4)))
    pygame.draw.polygon(windowSurface, bgColor, ((width,height/2), (width-width/6, height/2+height/4), (width-width/6, height/2-height/4)))

    # draw final fixation point
    if prime : fix_color=fixColor
    if not prime : fix_color=bgColor
    pygame.draw.circle(windowSurface,fix_color,winRect.center,int(winRect.width*.05))
...
pygame.init()
# set up the window
windowSurface = pygame.display.set_mode((1024,768), 1, 32)
pygame.display.set_caption('Sentences Example')
# set up fonts
basicFont = pygame.font.SysFont(None, 48)
basicBigFont = pygame.font.SysFont(None, 48*2)
# set up pointlists for arrows
winRect=windowSurface.get_rect()   
width = winRect.width
height = winRect.height
pointlist_left = ((0,height/2), (width/6, height/2+height/4), (width/6, height/2-height/4))
pointlist_right = ((width,height/2), (width-width/6, height/2+height/4), (width-width/6, height/2-height/4))
